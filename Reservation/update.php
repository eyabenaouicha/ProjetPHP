...
if (!empty($_POST)) {
  ...
  $queryget = "select * from reservation where id=:id";
 $pdostmtget = $pdo->prepare($queryget);
 $pdostmtget->execute(["id" => $_GET["id"]]);
 $row = $pdostmtget->fetch(PDO::FETCH_ASSOC);
  $prixF = $prixFormule + floatval($prixpc[0]["prix"]);
  $queryFact = "update facture set (prix=:prix) where id=:id";
  $pdostmtFact = $pdo->prepare($queryFact);
  $pdostmtFact->execute(["prix" => $prixF,"id" => $row["idFacture"]]);
  ...
  $pdostmtadd->execute(["idUtilisateur" => $_POST["inputUtilisateur"], "idChambre" => $_POST["inputChambre"], "iddateDebut" => $_POST["inputdateDebut"], "dateFin" => $_POST["inputdateFin"], "formule" => $_POST["inputformule"], "facture" => $idf, "etat" => $_POST["inputetat"], "id" => $_POST["inputid"]]);
